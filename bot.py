...
Admin commands (send these directly to the bot in your own DM)
  ...
  /emojiid <premium emoji>                 get a premium emoji's custom_emoji_id, to use in PREMIUM_EMOJI_IDS
"""
...
async def start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if is_admin(update.effective_user.id):
        text += (
            ...
            "/products, /deactivate, /reply, /emojiid."
        )
...
# -------------------------------------------------------------- admin: reply
async def admin_emoji_id(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """
    In the admin chat: /emojiid <premium emoji>  ->  replies with that
    emoji's custom_emoji_id, ready to paste into the PREMIUM_EMOJI_IDS env
    var. Requires the admin sending it to have Telegram Premium (so their
    client can actually pick a custom emoji to attach to the message).
    """
    ...
    if not ids:
        await update.message.reply_text(
            "Send /emojiid followed by the premium emoji you want the id for, e.g.:\n"
            "/emojiid 🔥\n"
            "(pick it from your Premium emoji panel, not the regular emoji keyboard)"
        )
        return
    lines = "\n".join(f"`{i}`" for i in ids)
    await update.message.reply_text(
        f"Custom emoji id(s):\n{lines}\n\n"
        "Paste one of these into PREMIUM_EMOJI_IDS on Render, e.g.:\n"
        f'`{{"buy": "{ids[0]}"}}`',
        parse_mode=ParseMode.MARKDOWN,
    )
...
def main():
    ...
    app.add_handler(CommandHandler("emojiid", admin_emoji_id))
